...
from app.settings import (
    ...
    SHOOT_COOLDOWN, ENEMY_TANK_HP, ENEMY_TANK_MOVING_POWER,
)
...
class EnemyTank(Entity):
    ...
    def _update_idle(self, dt):
        ...
        if d == 'up':
            self.moving_power_y = -ENEMY_TANK_MOVING_POWER
        elif d == 'down':
            self.moving_power_y = ENEMY_TANK_MOVING_POWER
        elif d == 'left':
            self.moving_power_x = -ENEMY_TANK_MOVING_POWER
        elif d == 'right':
            self.moving_power_x = ENEMY_TANK_MOVING_POWER
    ...
    def _update_combat(self, dt, target_pos, player_bullets):
        ...
        if dist > ideal_dist + dist_margin:
            ...
            if abs(dx) > self.width:
                self.moving_power_x = ENEMY_TANK_MOVING_POWER if dx > 0 else -ENEMY_TANK_MOVING_POWER
            if abs(dy) > self.height:
                self.moving_power_y = ENEMY_TANK_MOVING_POWER if dy > 0 else -ENEMY_TANK_MOVING_POWER
        ...
        for b in player_bullets:
            ...
            if b.vx != 0 and dist_y < self.height and dist_x < danger_dist:
                if (b.vx > 0 and bx < cx) or (b.vx < 0 and bx > cx):
                    self.moving_power_y = ENEMY_TANK_MOVING_POWER if cy < by else -ENEMY_TANK_MOVING_POWER
                    return
            ...
            if b.vy != 0 and dist_x < self.width and dist_y < danger_dist:
                if (b.vy > 0 and by < cy) or (b.vy < 0 and by > cy):
                    self.moving_power_x = ENEMY_TANK_MOVING_POWER if cx < bx else -ENEMY_TANK_MOVING_POWER
                    return
